    ...
    check: function(file, errors) {
        ...
        file.iterateNodesByType(['BlockStatement', 'ClassBody'], function(node) {
            ...
            if (setting === true || setting.indexOf(getBlockType(node)) !== -1) {
                ...
                var parentNodeFirstToken = file.getFirstNodeToken(parentNode);
                var parentNodeLastToken = file.getLastNodeToken(parentNode);
                var openingRoundBrace = file.findNextToken(parentNodeFirstToken, 'Punctuator', '(');
                var closingRoundBrace = file.findPrevToken(parentNodeLastToken, 'Punctuator', ')');
